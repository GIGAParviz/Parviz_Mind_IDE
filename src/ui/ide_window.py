...
class SimpleIDE(QMainWindow):
    ...
    def run_code(self):
        ...
        try:
            ...
            if not hasattr(self, 'terminal_widget') or not self.terminal_widget:
                ...
                if hasattr(self, 'terminal_minimize_btn'):
                    self.terminal_minimize_btn.setText("▲")
                    self.terminal_minimize_btn.setToolTip("Hide Terminal")
    ...
    def create_interactive_terminal(self):
        ...
        self.terminal_minimize_btn = QPushButton("▲")
        self.terminal_minimize_btn.setMaximumWidth(25)
        self.terminal_minimize_btn.setToolTip("Minimize Terminal")
        self.terminal_minimize_btn.setStyleSheet("""
            background-color: transparent;
            border: none;
            color: #E0E0E0;
            font-size: 12px;
            font-weight: bold;
        """)
        self.terminal_minimize_btn.clicked.connect(self.toggle_terminal)
        terminal_header_layout.addWidget(self.terminal_minimize_btn)
        
        self.terminal_close_btn = QPushButton("×")
        self.terminal_close_btn.setMaximumWidth(25)
        self.terminal_close_btn.setToolTip("Close Terminal")
        self.terminal_close_btn.setStyleSheet("""
            background-color: transparent;
            border: none;
            color: #E0E0E0;
            font-size: 12px;
            font-weight: bold;
        """)
        self.terminal_close_btn.clicked.connect(self.close_terminal)
        terminal_header_layout.addWidget(self.terminal_close_btn)
        ...
        try:
            if not hasattr(self, 'editor_terminal_splitter'):
                editor_widget = self.editor.parent()
                editor_layout = editor_widget.layout()
                self.editor_terminal_splitter = QSplitter(Qt.Orientation.Vertical)
                editor_layout.removeWidget(self.editor)
                
                self.editor_terminal_splitter.addWidget(self.editor)               
                self.editor_terminal_splitter.addWidget(terminal_widget)                
                self.editor_terminal_splitter.setSizes([700, 200])
                editor_layout.addWidget(self.editor_terminal_splitter)
            else:
                self.editor_terminal_splitter.addWidget(terminal_widget)
                self.editor_terminal_splitter.setSizes([700, 200])
    ...
    def toggle_terminal(self):
        ...
        if self.terminal_widget.isVisible():
            ...
            if hasattr(self, 'terminal_minimize_btn'):
                self.terminal_minimize_btn.setText("▼")
                self.terminal_minimize_btn.setToolTip("Show Terminal")
        else:
            self.terminal_widget.show()
            if hasattr(self, 'terminal_minimize_btn'):
                self.terminal_minimize_btn.setText("▲")
